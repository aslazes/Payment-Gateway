...
│
▼
Оформление заказа в магазине (Merchant)
│
│ 1. Создать платеж (POST /payments)
▼
Payment Gateway API
│
├── Payment Service
│ │
│ └─► Отправка запроса к провайдеру (Acquirer)
│
├── Payment Status Service
│ │
│ └─► Получение результата платежа (успешно/отклонено)
│
└── Refund Service
│
└─► Обработка возврата (POST /refunds)
│
▼
Сохранение статусов и публикация событий в Kafka
│
▼
Интернет-магазин получает результат
│
▼
Покупатель видит итоговый статус оплаты
